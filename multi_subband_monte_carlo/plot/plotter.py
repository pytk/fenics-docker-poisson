...
def electronDistribution(device, electron_distribution, iterate):
    ...
    X = np.linspace(0, device.xfi, device.nx+1)
    Z = np.linspace(0, device.zfi, device.nz+1)
    X, Z = np.meshgrid(X, Z)
    fig = plt.figure()
    plt.pcolor(X, Z, electron_distribution, norm=LogNorm(vmin=electron_distribution.min(), vmax=electron_distribution.max()), cmap=plt.cm.coolwarm)
    plt.colorbar()

    plt.savefig("/home/fenics/shared/multi_subband_monte_carlo/img/density/electron_distribution" + "_" + str(iterate) + ".png")